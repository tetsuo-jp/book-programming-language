...
data Instr = PushNum Int | PushVar Ide | PopVar Ide -- p.186
           ...
           | Stop
             deriving Show
-- data Expr = Num [Char]          -- p.187
--           | Pexpr Expr Expr
--           | Mexpr Expr Expr
type Prog = Expr
data Expr = Num Numeral         -- p.194
          | Pexpr Expr Numeral
          | Mexpr Expr Numeral
...
expr' :: Parser Char [Expr->Expr]
expr' =   (literal '+' `x_seq` numeral
          `using` uncurry (flip Pexpr))
    `seqp` expr'' `using` uncurry (:)
    `alt` (literal '-' `x_seq` numeral
          `using` uncurry (flip Mexpr))
    `seqp` expr'' `using` uncurry (:)
    `alt` succeed []
