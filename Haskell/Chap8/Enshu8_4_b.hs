...
cmdexec (Call x es) r p s                   -- p.178-179, p.164
    = alpha (map (\e -> expexec e r) es) [] s
    where
    V_Proc u = lookup r x
    alpha (f:fs) cs = f (\(V_Int v) -> alpha fs (cs++[V_Int v]))
    alpha []     cs = u cs p
...
vardecl (ProcDecl x ys c) (r,s) = (r',s) -- p.179, p.164, p.165
    where
    r' = update r x (V_Proc u)
    u vs p s = cmdexec c r'' p s''
                where
                (r'',s'') = foldr alpha (r',s) (zip ys vs)
                    where
                    alpha (y,v) (r0,s0) = let (l, (l', t')) = alloc s0 1
                                          in (update r0 y (V_Loc l),
                                              (l', update t' l v))
