module Sec8_1_d where             -- 閉包呼び, Extended to Sec8_1_c, Sec8_1_b, Sec8_1_a, Enshu6_7
...
cmdexec (Call x [e]) r p s                   -- p.170
    = u [V_Thunk (expexec e r)] p s
      where
      V_Proc u = lookup r x
...
vardecl (ProcDecl x [y] c) (r,s) = (r',s) -- p.167
    where
...
-- Expression (R-value), p.99, p.102, p.167
expval :: Expr -> Env -> State -> Val
expval (Num n) r s = V_Int (numval n)
expval (Var x) r s@(_,t) =      -- p.167
       case lookup r x of
         V_Loc k -> lookup t k
         v       -> v
...
-- Expression (L-value), p.167, p.102, Sec8_1_cでは使用されない
explval :: Expr -> Env -> State -> Val
explval (Num n) r s = undefined
explval (Var x) r s@(_,t) =     -- p.167
        case lookup r x of
          v@(V_Loc _) -> v
          _           -> undefined
...
alloc (l,t) n = (l, (l+n,t'))
        where
          t' = foldl (flip flip undefined . update) t [l..l+n-1] -- 修正
               where undefined = error "undefined in alloc"
...
testprog3 = "\
             ...
             \end\
             \"                 -- "
test14 = parse testprog3
test15 = exec testprog3
...
testprog4 = "\
             ...
             \    n := 10; i := 0; z := 0; call sum (a[i] * b[i]); output z \
             \end\
             \"                 -- "
test16 = parse testprog4
test17 = exec testprog4