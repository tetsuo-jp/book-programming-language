module Enshu8_4_a where    -- 変数呼び，Extended to Enshu8_2_a, Sec8_1_a, Enshu6_7
...
cmdexec (Call x es) r p s                    -- p.178-179, p.178, p.164
    = alpha (map (\e -> explexec e r) es) [] s
    where
    V_Proc u = lookup r x
    alpha (f:fs) cs = f (\(V_Loc k) -> alpha fs (cs++[V_Loc k]))
    alpha []     cs = u cs p
-- cmdexec (Call x [e1,e2,e3]) r p s                    -- p.178-179, p.178, p.164
--     = explexec e1 r q1 s
--     where
--     V_Proc u = lookup r x
--     q1 = \(V_Loc k1) -> explexec e2 r
--             (\(V_Loc k2) -> explexec e3 r
--                (\(V_Loc k3) -> u [V_Loc k1,V_Loc k2,V_Loc k3] p))
-- cmdexec (Call x es) r p s                    -- p.178-179, p.178, p.164
--       = u (map (\e -> explval e r s) es) p s
--       where
--       V_Proc u = lookup r x
...
vardecl (ProcDecl x ys c) (r,s) = (r',s) -- p.178-179, p.164
    where
    r' = update r x (V_Proc u)
    u vs = cmdexec c (foldr (\(y,v) r -> update r y v) r' (zip ys vs))
    ...
    = expexec e r q'
      where
        q' (V_Int v) = q (V_Loc (k + v))
          where
          V_Loc k = lookup r x
explexec (SeqExpr es) r q = explexec (last es) r q
...
decl = lit "var" `x_seq`
       ...
       ((lit "proc" `x_seq` kind T_Ide `seq_x` lit "(")         -- p.179
             `seqp` f_args `seq_x` lit ")" `seq_x` lit "=") `seqp`
          cmd
                        `using` (\((f,as),c) -> [ProcDecl f as c])
    where
    f_args = kind T_Ide `seqp`
                many (lit "," `x_seq` kind T_Ide) `using` uncurry (:) `alt`
             succeed []         -- p.179
...
cmd =	lit "begin" `x_seq` decls `seqp` seqcmd `seq_x` lit "end"
        ...
        ((lit "call" `x_seq` kind T_Ide `seq_x` lit "(")
                 `seqp` args `seq_x` lit ")" -- p.178-179
            `using` \(p,es) -> Call p es) `alt`
    ...
    where
    args =  expr `seqp` many (lit "," `x_seq` expr) `using` uncurry (:) `alt`
            succeed []              -- p.178-179
...
-- プロシージャ呼出しの引数の数を変化
test20 = parse "call f()"
test21 = parse "call f(a)"
test22 = parse "call f(a,b,c)"
test23 = parse "call f(a+1)"
test24 = parse "call f(a+1,b*2,c/3)"

testprog2 = "\
             \begin var z, w;\
             \    proc addsub(x,y) = begin x := x + y; y := x - 2 * y end;\
             \    z := 2; w := 3; call addsub(z,w); output z; output w \
             \end\
             \"              -- "
test30 = parse testprog2
test31 = exec testprog2

test41 = parse "begin var x; func f() = begin x := x+1; return x end; x := 0; output f(); output f() end" -- p.131
test42 = exec  "begin var x; func f() = begin x := x+1; return x end; x := 0; output f(); output f() end" -- p.131

-- プロシージャ呼出しの実引数に副作用がある場合
testprog3 = "\
             \begin var z, w, v;\
             \    proc f(x,y) = begin v := x+y end;\
             \    z := 3; w := 2; call f((z := 3; z), (w := 2; w)); output v \
             \end\
             \"                 -- "

test51 = parse testprog3
test52 = exec testprog3

-- 引数が0のプロシージャの場合
testprog4 = "\
             \begin\
             \    var z;\
             \    proc f() = begin z := 1 end;\
             \    call f(); output z \
             \end\
             \"                 -- "
test61 = parse testprog4
test62 = exec testprog4