...
-- Testing
test1 = parse"begin var x; func f () = (x := x + 1; x); x := 0; output f (); output f () end" -- p.125
test2 = parse"begin var x; func f () = (x := x + 1; x) end"
test3 = lex  "begin func f () = 3; output 3 end"
test4 = parse"begin func f () = 3; output 3 end"

test5 = (kind T_Ide `seq_x` lit "(" `seq_x` lit ")" `seq_x` lit "=") [(T_Ide,"f"),(T_Sym,"("),(T_Sym,")"),(T_Sym,"=")]
test6 = (lit "func" `x_seq` kind T_Ide `seq_x` lit "(" `seq_x` lit ")" `seq_x` lit "=") [(T_Sym,"func"),(T_Ide,"f"),(T_Sym,"("),(T_Sym,")"),(T_Sym,"=")]
test7 = ((lit "func" `x_seq` kind T_Ide `seq_x` lit "(" `seq_x` lit ")" `seq_x` lit "=") `seqp` expr) [(T_Sym,"func"),(T_Ide,"f"),(T_Sym,"("),(T_Sym,")"),(T_Sym,"="),(T_Num,"3")]
test8 = ((lit "func" `x_seq` kind T_Ide `seq_x` lit "(" `seq_x` lit ")" `seq_x` lit "=") `seqp` expr `using` (\(f,e) -> FuncDecl f [] e)) [(T_Sym,"func"),(T_Ide,"f"),(T_Sym,"("),(T_Sym,")"),(T_Sym,"="),(T_Num,"3")]
test9 = decl' [(T_Sym,"func"),(T_Ide,"f"),(T_Sym,"("),(T_Sym,")"),(T_Sym,"="),(T_Num,"3")]
test10= decl $ lex "func f () = 3"
test11= cmd $ lex "output 3"
test12= decls $ lex "func f () = 3;"

test13= exec "begin func f () = 3; output 3 end"
test14= exec "begin var x; func f () = (x := x + 1; x); x := 0; output f (); output f () end" -- p.125

test15= exec "begin var x; proc p () = begin x := x + 1 end; x := 0; call p (); output x; call p (); output x end"
test16= parse"begin var x; proc p () = begin x := x + 1 end; x := 0; call p (); output x; call p (); output x end"
test17= decl $ lex "proc p () = begin x := x + 1 end"
test18= decls $ lex "proc p () = begin x := x + 1 end;"
test19= decls $ lex "var x; proc p () = begin x := x + 1 end;"
test20= cmd $ lex "begin var x; proc p () = begin x := x + 1 end; output 3 end"
test21= cmd $ lex "call p ()"
test22= cmd $ lex "begin var x; proc p () = begin x := x + 1 end; call p () end"