...
cmd =	lit "begin" `x_seq` decls `seqp` seqcmd `seq_x` lit "end"
        ...
        lit "continue" `using` const Continue -- p.130 演習問題6.6
...
-- Testing
test1 = parse "continue"
test2 = exec "begin var n; n := 3; while n>0 do begin n:=n-1; output n; continue; n := 99 end; output n end"
test3 = exec "begin var a[10]; a[8] := 5; output a[8] end"